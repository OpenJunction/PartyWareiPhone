...
- (IBAction)searchYouTube {
	[self presentModalViewController:searchViewController animated:YES];
	 ...
	 [query setVideoQuery:@"night"];
	 
	 GDataServiceTicket *ticket = [youTubeService fetchFeedWithQuery:query
	 delegate:self
...
- (void)ticket:(GDataServiceTicket *)ticket finishedWithFeed:(GDataFeedYouTubeVideo *)feed
	...
	if (error) {
		...
		NSLog(@"---Feed Entries---");
		for (GDataEntryYouTubeVideo *video in [feed entries]) {
			NSLog(@"%@", [[video title] stringValue]);
		}
	}
}
*/
